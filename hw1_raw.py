...
def preprocess4classification(raw, random_state=42):
    entries = ['school', 'sex', 'age', 'famsize', 'studytime', 'failures', 'activities',
                'higher', 'internet', 'romantic', 'famrel', 'freetime', 'goout', 'Dalc',
                'Walc', 'health', 'absences', 'G3']
    want = raw[entries]
    want[['label']] = want[['G3']].apply(lambda x: [1 if y >= 10 else 0 for y in x])
    want = want.drop(columns=['G3'])
...
def Problem1():
    ...
    # predicted.append( ('Linear Regression', lr.predict(X_test)) )
    # w.append(lr.weights)
    ...
    # predicted.append( ('Linear Regression (/reg)', lr_reg.predict(X_test)) )
    ...
    # predicted.append( ('Linear Regression (/b)', lr_b.predict(X_test)) )
    ...
    # predicted.append( ('Linear Regression (r/b)', lr_b_reg.predict(X_test)) )
    ...
    # predicted.append( ('Bayesian Linear Regression', blr.predict(X_test)) )

    # plot_G3(y_test, predicted)
    # plot_weights(w)

    # raw = pd.read_csv('train.csv')
    # preprocess4classification(raw)

def Problem2():
    raw = pd.read_csv('train.csv')
    X_train, X_test, y_train, y_test = preprocess4classification(raw)
